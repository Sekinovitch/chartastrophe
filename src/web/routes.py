...
@bp.route('/api/dataset-translations')
def get_dataset_translations():
    """Retrieve dataset translations for the current language."""
    try:
        lang = get_user_language()
        translations = {}
        
        # Only load translations for French
        if lang == 'fr':
            import json
            ...
            if pretranslated_file.exists():
                ...
                logger.debug(f"Loaded {len(translations)} dataset translations for French")
        else:
            logger.debug(f"No dataset translations loaded for language: {lang}")
        ...
        logger.error(f"Error loading dataset translations: {str(e)}")
        return jsonify({
            'status': 'error',
            'message': 'Could not load dataset translations',
            'translations': {},
...
def get_random_correlation():
    try:
        ...
        # Explanation is already generated by CorrelationEngine with correct language
        logger.debug("Using explanation from CorrelationEngine")
        
        # Add fallback explanation if none exists
        if 'explanation' not in correlation or not correlation['explanation']:
            user_lang = get_user_language()
            logger.warning("No explanation found, generating fallback")
            if user_lang == 'fr':
